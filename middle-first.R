...
filename <- c()
recorder.ea.global <- c(); recorder.ea.local <- c(); recorder.ea.global.all <- c(); recorder.ea.local.all <- c(); recorder.ea.first <- c()
recorder.af.global <- c(); recorder.af.local <- c(); recorder.af.global.all <- c(); recorder.af.local.all <- c(); recorder.af.first <- c()
recorder.sa.global <- c(); recorder.sa.local <- c(); recorder.sa.global.all <- c(); recorder.sa.local.all <- c(); recorder.sa.first <- c()
...
for ( i in 1:len.csv.list ) {
  ...
  for ( j in 1:len.area ) {
    if( dat$area[j] == "East Asia" && dat$status[j] == "middle" ){
      for ( k in (j-1):1 ){
        if( dat$area[j] != "East Asia" && dat$status[k] == "first" ){ break
        } else if( dat$area[j] == "East Asia" && dat$status[k] == "first" ){
