...
for ( i in 1:len.csv.list ) {
  ...
  dat <- read.csv( csv_list[i] )
  num.list01 <- c( which(dat$area_1=="Africa"), which(dat$status=="middle") )
  num.non.corresponding.author.af01 <- num.list01[duplicated( num.list01 )]
  num.list02 <- c( which(dat$area_1=="Africa"), which(dat$status=="last") )
  num.non.corresponding.author.af02 <- num.list02[duplicated( num.list02 )]
  num.non.corresponding.author.af <- c( num.non.corresponding.author.af01, num.non.corresponding.author.af02 )
...
for ( i in 1:len.csv.list ) {
  ...
  dat <- read.csv( csv_list[i] )
  num.list <- c( which(dat$area_1=="Africa"), which(dat$status=="first") )
