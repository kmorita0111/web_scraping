...
for ( i in 1:len.csv.list ) {
  ...
  dat <- read.csv( csv_list[i] )
  num.list01 <- c( which(dat$area_1=="South America"), which(dat$status=="middle") )
  num.non.corresponding.author.sa01 <- num.list01[duplicated( num.list01 )]
  num.list02 <- c( which(dat$area_1=="South America"), which(dat$status=="last") )
  num.non.corresponding.author.sa02 <- num.list01[duplicated( num.list02 )]
  num.non.corresponding.author.sa <- c( num.non.corresponding.author.sa01, num.non.corresponding.author.sa02 )
...
for ( i in 1:len.csv.list ) {
  ...
  dat <- read.csv( csv_list[i] )
  num.list <- c( which(dat$area_1=="South America"), which(dat$status=="first") )
